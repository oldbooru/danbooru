class PoolController < ApplicationController
  ...
  def show
    ...
    @posts = Post.paginate :per_page => 24, :order => "pools_posts.sequence, pools_posts.post_id", :joins => "JOIN pools_posts ON posts.id = pools_posts.post_id", :conditions => ["pools_posts.pool_id = ? AND posts.status <> 'deleted' AND posts.rating = 's'", params[:id]], :select => "posts.*", :page => params[:page]
